import numpy as np 
import msggan.spec_ops as spec_ops
import msggan.phase_operation as phase_op


def _linear_to_mel_matrix(_sample_rate=16000, num_freq_bins=1024):
    """Get the mel transformation matrix."""
    _mel_downscale = 1
    lower_edge_hertz = 0.0
    upper_edge_hertz = _sample_rate / 2.0
...
def _mel_to_linear_matrix(_sample_rate=16000, num_freq_bins=1024):
    """Get the inverse mel transformation matrix."""
    m = _linear_to_mel_matrix(_sample_rate, num_freq_bins)
    m_t = np.transpose(m)
    p = np.matmul(m, m_t)
...
def melspecgrams_to_specgrams(logmelmag2, mel_p, _sample_rate=16000, num_freq_bins=1024):
    ...
    mel2l = _mel_to_linear_matrix(_sample_rate, num_freq_bins)
...
def specgrams_to_melspecgrams(magnitude, IF, _sample_rate=16000, num_freq_bins=1024):
    ...
    l2mel = _linear_to_mel_matrix(_sample_rate, num_freq_bins)
